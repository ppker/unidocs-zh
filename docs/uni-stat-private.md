...
在 `manifest` 的 **uni统计** 中开启 **uni统计2.0 私有版** 并发行业务 App，在 [uniCloud](https://doc.dcloud.net.cn/uniCloud/) 服务空间部署 [uni-admin](https://doc.dcloud.net.cn/uniCloud/admin) 后即可查看报表。
...
无需在各端接入不同 SDK、无需在不同报表间切换。目前市面已知唯一一张报表看遍业务全景的方案。
...
`uni统计2.0 私有版` 基于 `uniCloud` 实现，云函数与统计数据全部托管在开发者自己的服务空间（阿里云、腾讯云或支付宝云等，按需选择）中，开发者对统计数据拥有完整控制权。
...
- uni-app 全端识别，无需对接不同 SDK、无需在不同报表间切换并自行累加数据
...
**DCloud 非常重视数据驱动开发者业务增长，近期联合专业大数据服务商 <a href="https://www.sensorsdata.cn/" target="_blank">神策</a> 与 <a href="https://tj.aldwx.com/" target="_blank">阿拉丁</a> 共同完善 uni统计2.0 的产品模块与生态建设。
如果你对数据分析有任何需求，欢迎填写 <a href="https://sdmarketing.wjx.cn/vj/h2UFHnx.aspx" target="_blank">需求调研问卷</a>，我们将根据您的建议持续迭代 uni统计 产品。**


## 老版升级与版本选择

`uni统计1.0` 已不再更新维护，推荐尽快升级至 `uni统计2.0`：

| 场景 | 推荐版本 |
| :-: | :-: |
| 免部署、登录即用、数据由 DCloud 托管 | [uni统计2.0 公有版](uni-stat-public.md) |
| 数据自控、报表与跑批部署在自有 uniCloud | **uni统计2.0 私有版**（本文档） |

与 `uni统计1.0` 的能力对比（以私有版为例）：
...
- `enable`：`true` 开启、`false` 关闭统计。
- `type`：设为 `"private"` 表示私有版（`HBuilderX 5.13+` 推荐）；旧项目可继续使用 `version: "2"`，与 `type: "private"` 等价。
- 与公有版二选一：开启私有版时请确保未同时开启 `type: "public"`。
...
|字段|类型|默认值|可选值|说明|
|:-:|:-:|:-:|:-:|:-:|
|enable|Boolean|false|true，false|全局开启或关闭统计；分平台 `enable` 会覆盖本项|
|type|String|"public"|"public"，"private"|`private`：私有版；`public`：公有版。与 `version` 二选一配置即可|
|version|String|"1"|"1"，"2"|旧版配置项，`"2"` 表示 uni统计2.0 私有版；`HBuilderX 5.13+` 建议使用 `type`|
|debug|Boolean|false|true，false|调试模式，会产生大量日志，应用发布请关闭|
|reportInterval|Number|10|-|前端数据上报周期，单位：秒（**HBuilderX 3.5.4+**）|
|collectItems|Object|-|-|采集项配置（**HBuilderX 3.5.5+**）|
...
`uniStatistics` 支持分平台配置。例如仅在微信小程序开启统计，在 `manifest.json` 的 `mp-weixin` 节点下设置 `uniStatistics -> enable`：
...
|字段|类型|默认值|可选值|说明|
|:-:|:-:|:-:|:-:|:-:|
|enable|Boolean|false|true，false|分平台开启或关闭统计；覆盖全局 `enable`|
...
`uni统计2.0 私有版` 的统计报表是 [uni-admin](https://uniapp.dcloud.io/uniCloud/admin.html) 的内置插件。

[uni-admin](https://uniapp.dcloud.io/uniCloud/admin.html) 是开源管理后台。使用私有版须部署该后台，在左侧菜单进入 uni统计 查看报表。
...
4. 在云端配置中心完成其它初始化配置，如：在 `uniCloud/cloudfunctions/common/uni-config-center/uni-id/config.json` 文件中填写自己的 passwordSecret 字段 (用于加密密码入库的密钥) 和 tokenSecret 字段 (为生成 token 需要的密钥，测试期间跳过本条也可以)，然后对`uni-config-center`公共模块点右键上传更新。注意：`业务端App项目`和`报表端uni-admin项目`关联相同服务空间时，可能会出现`uni-config-center`的相互覆盖问题，此时建议单点维护，[详见](#常见问题)。
...
`uni统计2.0 私有版` 中 **定时任务云函数（uni-stat-cron）** 默认每小时触发一次（即每隔 1 小时自动执行跑批）。
...
::: warning 注意
开启 redis 缓存前，须确认已在**部署** uni统计 的服务空间内购买 redis；未购买须先开通。
:::

**开启步骤：**
1. 修改[uni统计配置项](#公共模块配置项说明)将`redis`参数的值改为`true`。
2. 分别在 **上报数据接收器（uni-stat-receiver）** 与 **定时任务云函数（uni-stat-cron）** 的 `package.json` 中添加 redis **扩展库**。
3. 重新上传部署数据`上报数据接收器（uni-stat-receiver）`、`定时任务云函数（uni-stat-cron）`和`配置中心（uni-config-center）`。

::: preview
> 配置 uni-stat-receiver 的 redis 扩展库
``` javascript
{
...
- 前端采集SDK源码地址，[查看](https://github.com/dcloudio/uni-app/tree/uni-app-vue3-dev/packages/uni-stat)
- 云端统计模块以及统计后台源码地址，[查看](https://github.com/dcloudio/uni-admin)

### 前端采集SDK @web-sdk
#### 数据上报逻辑 @report-time

数据默认每 **10 秒** 上报一次。上报间隔内，各采集节点的数据先进入队列，在下一上报节点统一处理后发送。

为节约服务端资源，前端按固定周期上报，**并非实时逐条上报**。

`HBuilderX 3.5.4+` 可在 **uni统计配置 → 前端数据上报周期** 中调整周期，建议取值 **5–20**（秒）。

::: warning 什么时候会上报数据？

- **发行**后运行到已开通统计的平台（`HBuilderX → 发行 → 选择对应平台`）
- **运行**且 `manifest.json → uniStatistics → debug: true`（调试模式）

除以上两种情况外**不会上报**（含 App 真机运行、自定义基座等；崩溃与错误数据亦同此规则）。
...
- `基础会话日志：sessionLog`，默认保留 `31` 天。注意：设备留存最长需统计 30 天后的数据，基础会话日志至少保留 `31` 天，否则影响设备留存统计。
- `用户会话日志：userSessionLog`，默认保留 `31` 天。注意：用户留存最长需统计 30 天后的数据，用户会话日志至少保留 `31` 天，否则影响用户留存统计。
...
## 扩展和自定义方式
uni统计 提供基础数据报表。若默认采集无法满足需求，可在客户端通过 `uni.report(eventKey, param)` 自定义上报，并在 uni-admin 中扩展页面自行统计展示。
...
如果你已启用 `uni统计2.0 私有版`，但使用的是旧版 `uni-admin`，需注意版本表迁移升级，否则基于版本的统计数据可能不准确。
...
uni-admin项目更新后，即可正常接收和统计`App-Harmony`平台的相关数据。此外，还须升级 **uni统计2.0 私有版** 的前端采集 SDK：将 `HBuilderX` 升级至 `4.33+` 后，重新打包发布待统计的应用。
...
uni-admin项目更新后，即可正常接收和统计`鸿蒙元服务`平台的相关数据。此外，还须升级 **uni统计2.0 私有版** 的前端采集 SDK：将 `HBuilderX` 升级至 `4.33+` 后，重新打包发布待统计的应用。


## uni统计2.0 私有版费用评测@cost

### 前言

部分开发者对基于uniCloud的`uni统计`等云端一体业务，不清楚这些业务预计会花费多少钱，不清楚相比传统服务器而言，何种方案性价比更好。

本文尝试算细账、算总账，以支付宝云 [按量计费](https://doc.dcloud.net.cn/uniCloud/price.html#alipay-postpay) 为例，预测 `uni统计2.0 私有版` 在不同用户规模下的资源消耗与费用，供选型参考。
...
::: warning 注意
以下资源消耗计算均以支付宝云服务空间为准，阿里云及腾讯云计费规则不一样。
:::
...
我们按照[uniCloud官网](https://doc.dcloud.net.cn/uniCloud/price.html#alipay-postpay)列出的按量计费规则，计算一下云函数的资源消耗。
...
即：App 日活为 100、启用 `uni统计` 后，`uni-stat-receiver` 云函数每天约消耗 0.0032 元。
...
即：App 日活为 100、启用 `uni统计` 后，`uni-stat-receiver` 云函数每天约消耗 0.0014 元。
...
按照[uniCloud官网](https://doc.dcloud.net.cn/uniCloud/price.html#alipay-postpay)介绍，云数据库费用 = `容量费用 + 读操作次数费用 + 写操作次数费用`，其中：
...
即：`uni统计` 业务日活为 100、使用阿里云正式版云服务空间时，数据库每天约消耗 0.0141 元。
...
按照[uniCloud官网](https://doc.dcloud.net.cn/uniCloud/price.html#alipay-postpay)介绍，前端网页托管费用 = `容量费 + 流量费`。
...
- 2.开启redis缓存
这也是目前降低 `uni统计2.0 私有版` 数据库查询次数最有效的方法。redis 按容量计费，读写次数增多不会额外按次收费。开启方法参见上文 [开启 redis 缓存](#开启redis缓存)。
- 3.关闭实时统计
...
`uni统计2.0 私有版` 统计数据的最小粒度为**小时**（每小时统计上一小时数据）。因此上报后约 1 小时方可在 `uni-admin` 看到结果。测试阶段若需快速出数，可在 `uni-admin` 项目下新建云函数，在 `HBuilderX` 中右键关联 `uni-stat` 公共模块，将下方示例代码粘贴到测试云函数，按需修改任务类型、运行时间、批次类型等参数后，右键云函数目录选择**本地运行云函数**即可。
...
exports.main = async (event, context) => {
	...
	let res = await uniStatCron.stat({
		type: 'stat', //任务类型，stat:基础统计、page:页面统计。。。，任务类型及跑批时间可查看文档(uni-stat-private.md#定时任务配置说明)
...
在 `HBuilderX` 中打开待统计项目，点击根目录 `manifest.json` 的**源码视图**，找到**最外层** `uniStatistics` 节点（不是各平台节点下的 `uniStatistics`），添加 `uniCloud` 配置项。服务空间信息可登录 [uniCloud 控制台](https://unicloud.dcloud.net.cn/)，在服务空间**详情**中查看。
...
答：与定时任务配置有关，默认`统计首页`、`今日概况`等数据为1小时后可见，其余数据为次日可见，测试期间可通过上方[在开发阶段快速生成统计数据](#在开发阶段快速生成统计数据)栏目中所述方法，手动触发统计任务，提前生成统计数据。想要详细了解各类型数据统计时间请参考[定时任务配置说明](#定时任务配置说明)。
...
- 确保业务 App 的 `manifest` 已开启 **uni统计2.0 私有版**，并关联正确的 uniCloud 服务空间
...
- 在 [uniCloud Web 控制台](https://unicloud.dcloud.net.cn/) 云函数日志中，确认 `uni-stat-cron` 定时执行成功；若出现 `Not Found the config file`，见下方第 6 条。
- 仅当需要统计 **uni-admin 管理端自身** 时，才在 `uni-admin` 工程的 `manifest` 中开启 **uni统计2.0 私有版** 并再次发行。**勿与业务 App 混淆。**
...
答：总设备数 = 原设备数 + 新设备数。是否为新设备取决于客户端 SDK 是否记录该设备上次访问时间（`lvts=0` 为新设备，`lvts>0` 为老设备）。若应用曾接入 uni统计1.0 或 uni统计2.0（含公有版、私有版），即使清空后台数据，客户端仍可能保留 `lvts>0`，该设备不会重新计入新设备，但总设备数不变、后续访问仍计入活跃，从而可能出现总设备数小于活跃设备数。
...
不掌握如下文档，很难吃透 `uni统计2.0 私有版` 并进行二次开发：
